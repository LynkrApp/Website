...
const AnimatedBackground = ({ theme, children }) => {
  ...
  const createMatrixRain = (container, theme) => {
    ...
    const chars = theme.speed === 'fast' ? '01' : '01アイウエオカキクケコサシスセソタチツテトナニヌネノハヒフヘホマミムメモヤユヨラリルレロワヲン';
    const animationDuration = theme.speed === 'fast' ? '2s' : theme.speed === 'slow' ? '4s' : '3s';
  ...
  const createPulseWaves = (container, theme) => {
    ...
    const waveInterval = theme.pulseSpeed === '1.5s' ? 800 : theme.pulseSpeed === '3s' ? 1800 : 1200;
  ...
  const createTypingEffect = (container, theme) => {
    const phrases = theme.phrases || ['LOADING...', 'INITIALIZING...', 'CONNECTING...', 'PROCESSING...'];
    let currentPhrase = 0;
    let currentChar = 0;
  ...
  const createDNAHelix = (container, theme) => {
    ...
    for (let turn = 0; turn < turns; turn++) {
      for (let point = 0; point < pointsPerTurn; point++) {
        ...
        const y = (turn / turns) * helixHeight + (point / pointsPerTurn) * (helixHeight / turns);
  ...
  const getBackgroundStyle = () => {
    ...
    if (theme.animation === 'gradient') {
      ...
      style.backgroundSize = theme.backgroundSize || '400% 400%';
    }
