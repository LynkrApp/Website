...
const EnhancedThemesPicker = () => {
  ...
  useEffect(() => {
    ...
    if (dbButtonStyle?.name) {
      ...
      const legacyButtonStyle = buttonStyles.find((t) => t.css === currentUser.buttonStyle);
      ...
      return (
        ...
        >
          <div className="space-y-2">
            ...
            <div className={`space-y-1 ${theme.containerWidth === 'max-w-sm' ? 'max-w-16' : theme.containerWidth === 'max-w-lg' ? 'max-w-20' : 'max-w-24'}`}>
              <div className={`h-2 bg-gray-300 rounded ${theme.cardStyle === 'pill' ? 'rounded-full' : theme.cardStyle === 'minimal' ? 'rounded-sm' : 'rounded'}`} />
              <div className={`h-2 bg-gray-300 rounded ${theme.cardStyle === 'pill' ? 'rounded-full' : theme.cardStyle === 'minimal' ? 'rounded-sm' : 'rounded'}`} />
              <div className={`h-2 bg-gray-300 rounded ${theme.cardStyle === 'pill' ? 'rounded-full' : theme.cardStyle === 'minimal' ? 'rounded-sm' : 'rounded'}`} />
            </div>
