...
const OgImageEditor = () => {
  ...
  const handleSubmit = () => {
    mutation.mutate(ogStyles as any);
  };
    ...
    <div className="w-full max-w-[100vw] overflow-hidden px-2 sm:px-4 mx-auto my-6">
      ...
      <div className="mb-6 overflow-hidden bg-white border shadow-sm rounded-xl">
        ...
        <div className="flex flex-col xs:flex-row items-center justify-between p-3 sm:p-4 border-t">
          <span className="text-xs sm:text-sm text-gray-500 mb-2 xs:mb-0">
            Preview of how your page will appear when shared
          </span>
          <button
      ...
      <div className="mb-6 overflow-hidden bg-white border rounded-xl">
        ...
        <div className="p-3 sm:p-4 md:p-6">
          ...
          {activeTab === 'background' && (
            <div>
              <h3 className="mb-4 text-base sm:text-lg font-medium">
                Background Style
              </h3>
                  ...
                  {backgroundTabs.map((bgType) => (
                    <button
                      ...
                      className={`p-2 sm:p-3 rounded-lg flex flex-col items-center gap-1 transition-all ${
                        ogStyles.backgroundType === bgType.id
              ...
              <div>
                <h4 className="mb-3 text-sm sm:text-base font-medium">
                  Text & Accent Colors
                </h4>
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  {/* Text Color */}
                  <div className="mb-4">
                    <label className="block mb-2 text-xs sm:text-sm text-gray-600">
                      Text Color
                    </label>
                ...
                <div className="grid grid-cols-1 gap-3 xs:grid-cols-2">
                  {['default', 'minimal', 'modern', 'bold'].map((template) => (
                    <button
                      ...
                      className={`p-3 sm:p-4 rounded-lg flex flex-col items-center gap-1 border-2 transition-all ${
                        ogStyles.template === template
                          ? 'bg-purple-50 border-purple-200'
                          : 'bg-gray-50 border-gray-100 hover:bg-gray-100'
                      }`}
                    >
                      <PanelTop
                        className={`${ogStyles.template === template ? 'text-purple-600' : 'text-gray-500'} w-5 h-5 sm:w-6 sm:h-6`}
                      />
                      <span
              ...
              <div className="mb-6">
                ...
                <div className="pr-1 sm:pr-2 space-y-2 overflow-y-auto max-h-60 sm:max-h-96">
                  {typographyThemes.slice(0, 6).map((font) => (
                    <button
                      key={font.name}
                      className={`w-full rounded-lg p-3 sm:p-4 text-left transition-colors ${
                        ogStyles.typographyTheme?.name === font.name
          ...
          {activeTab === 'display' && (
            <div>
              ...
              <div className="space-y-3 sm:space-y-4">
                {/* Show/Hide Avatar */}
                <div
                  className="flex items-center justify-between p-3 sm:p-4 transition-colors rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100"
                  onClick={() =>
                    handleChange('showAvatar', !ogStyles.showAvatar)
                  }
                >
                  <div className="flex items-center gap-2 sm:gap-3">
                    ...
                    <div>
                      <h4 className="text-sm sm:text-base font-medium">
                        {ogStyles.showAvatar ? 'Show Avatar' : 'Hide Avatar'}
                      </h4>
                  ...
                  <div
                    className={`w-10 sm:w-12 h-5 sm:h-6 rounded-full relative ${
                      ogStyles.showAvatar ? 'bg-green-500' : 'bg-gray-300'
                    }`}
                  >
                    <div
                      className={`absolute w-4 sm:w-5 h-4 sm:h-5 rounded-full bg-white top-0.5 transition-transform ${
                        ogStyles.showAvatar
                ...
                >
                  ...
                  <div
                    className={`w-10 sm:w-12 h-5 sm:h-6 rounded-full relative ${
                      ogStyles.showStats ? 'bg-green-500' : 'bg-gray-300'
                    }`}
                  >
                    <div
                      className={`absolute w-4 sm:w-5 h-4 sm:h-5 rounded-full bg-white top-0.5 transition-transform ${
        ...
        <div className="flex gap-2 sm:gap-3">
          ...
          <div>
            <p className="text-xs sm:text-sm text-blue-800">
              These settings control how your profile looks when shared on
              social media, in messages, and anywhere else links are previewed.
            </p>
          </div>
        </div>
      </div>
