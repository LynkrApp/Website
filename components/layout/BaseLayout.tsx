...
const BaseLayout = ({
  ...
  const enhancedNavbarProps: any = {
    ...activeTheme.navbar,
    ...navbarProps,
    className: `${glassmorphism ? 'glass-effect' : ''} ${(navbarProps as any).className || ''} ${(activeTheme.navbar as any).className || ''}`,
  };

  // Enhanced footer props
  const enhancedFooterProps: any = {
    ...activeTheme.footer,
    ...footerProps,
    className: `${(footerProps as any).className || ''} ${(activeTheme.footer as any).className || ''}`,
  };

  return (
    <>
      {/* Meta tags */}
      <MetaTags title={''} image={'/og.png'} url={'/'} {...metaProps} />
      ...
      <div
        className={`flex flex-col ${fullHeight ? 'min-h-screen' : ''} ${activeTheme.bg} ${activeTheme.text}`}
      >
        {/* Navbar - fixed at top */}
        {showNavbar && (
          <div className="sticky top-0 z-50">
            <Navbar
              transparent={(navbarProps as any).transparent || false}
              {...enhancedNavbarProps}
            />
          </div>
        )}
        ...
        <div className={`flex-1 ${patternClass}`}>
          ...
          <main
            className={`${showNavbar ? 'pt-10 md:pt-16' : ''} ${getAnimationClass()}`}
          >
