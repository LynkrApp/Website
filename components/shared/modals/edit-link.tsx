...
const EditLinkModal = ({
  ...
  const [newShowFavicon, setNewShowFavicon] = useState(
    showFavicon === true ? 'true' : 'false'
  );
  ...
  const switchRootClass =
    'w-[39px] h-[21px] bg-[#E4E4E7] rounded-full relative focus:shadow-black border border-slate-200 data-[state=checked]:bg-slate-900 outline-none cursor-default lg:w-[42px] lg:h-[25px]';
  const switchThumbClass =
    'block w-[17px] h-[17px] bg-white rounded-full shadow-[0_2px_2px] transition-transform duration-100 translate-x-0.5 will-change-transform data-[state=checked]:translate-x-[19px] lg:w-[21px] lg:h-[21px]';
              ...
              <div className="relative flex justify-between gap-2 p-2 my-4 text-gray-800">
                <div>
                  <h3 className="text-md lg:text-lg">
                    Show website icon (Coming Soon)
                  </h3>
                  <p className="text-xs text-gray-500">
                    Display the website's favicon next to the link title.
                    <br />
                    NOTE: this option is currently true by default.
                  </p>
                </div>
                <Switch.Root
                  checked={newShowFavicon === 'true'}
                  onCheckedChange={(val) =>
                    setNewShowFavicon(val ? 'true' : 'false')
                  }
