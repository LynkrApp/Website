...
const ProfilePage = ({ serverHandle }: ProfilePageProps) => {
  ...
  const buttonStyleTheme = user?.buttonStyleTheme;
  const theme = {
    primary: user?.themePalette?.palette[0],
    secondary: user?.themePalette?.palette[1],
    accent: user?.themePalette?.palette[2],
    neutral: user?.themePalette?.palette[3],
  };

  const getBackgroundStyle = (): React.CSSProperties => {
    const themePalette = user?.themePalette;
  ...
  const shouldShowOverlay = () => {
    const themePalette = user?.themePalette;
    return (
      (themePalette?.type === 'image' || themePalette?.type === 'pattern') &&
      themePalette?.overlay
    );
  };

  const getContainerClasses = () => {
    const layout = user?.layoutTheme;
    if (!layout)
      return 'relative z-10 flex flex-col items-center justify-center w-full max-w-2xl px-4 mx-auto mt-6 md:mt-10';

    const alignmentClass =
      layout.alignment === 'left' ? 'items-start' : 'items-center';
    const spacingClass =
  ...
  const getTypographyStyles = () => {
    const typography = user?.typographyTheme;
  ...
  return (
    <>
      ...
      <AnimatedBackground
        theme={
          (user?.themePalette?.type === 'animated'
            ? user?.themePalette
            : null) as any
        }
      >
        <section
          ...
          className={`min-h-screen w-full no-scrollbar overflow-auto relative ${
            user?.themePalette?.type === 'animated' &&
            user?.themePalette?.animation === 'gradient'
              ? 'animated-gradient'
              : ''
          }`}
        >
          {shouldShowOverlay() && (
            <div
              ...
              style={{
                backgroundColor: (user as any)?.themePalette?.overlay as string,
              }}
            />
          )}
          <div
          ...
          >
            ...
            <div className="flex flex-col items-center mb-4">
              ...
              <Avatar.Root className="inline-flex items-center justify-center w-32 h-32 overflow-hidden align-middle border-2 border-blue-300 rounded-full sm:h-36 sm:w-36">
                <Avatar.Image
            ...
            {user?.layoutTheme?.displayMode === 'tabbed' ? (
              <TabbedSections
                sections={sectionList as any}
                userLinks={links as any}
                theme={theme}
                ...
                {sectionList
                  ...
                  .map((section) => {
                    ...
                    return (
                      <div
                        key={section.id}
                        className="flex flex-col items-center w-full max-w-md my-3"
                      >
                        <h3
                          style={{
                            color: theme.accent,
            ...
            {links?.filter((link) => !link.archived).length === 0 && (
          ...
          {links?.filter((link) => !link.archived).length > 0 && (
            <footer className="w-full py-8 mt-4 text-center">
              <p style={{ color: theme.accent }} className="text-base">
                Made with{' '}
                <Link
