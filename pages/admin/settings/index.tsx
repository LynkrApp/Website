...
const Settings = () => {
  ...
  const { data: fetchedUser } = useUser(
    typeof currentUser?.handle === 'string' ? currentUser.handle : ''
  );
  ...
  }, [user?.name, user?.bio, user?.image, user?.handle]);

  // edit profile details
  const editMutation = useMutation<void, unknown, { bio: string; username: string; image: string; handle: string }>(
